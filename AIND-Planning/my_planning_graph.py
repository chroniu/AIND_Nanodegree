...
class PlanningGraph():
    ...
    def __init__(self, problem: Problem, state: str, serial_planning=True):
        ...
        self.all_= self.problem.actions_list + self.noop_actions(self.problem.state_map)
        self.s_levels = []
        self.a_levels = []
        self.create_graph()
...
#        new_a_level = dict()
        ...
        for action in self.all_ :
            pg_node_a = PgNode_a(action)
            for pg_node_s in self.s_levels[level]:
                if pg_node_s in pg_node_a.prenodes:
                    pg_node_s.children.add(pg_node_a)
                    pg_node_a.parents.add(pg_node_s)
            if len(pg_node_a.parents) != 0:
                self.a_levels[level].add(pg_node_a)
            
#                if( (pg_node_s.symbol in action.precond_pos) or
#                    (pg_node_s.symbol in action.precond_neg)):
#                    if action not in new_a_level:
#                        new_a_level[action] = set()
#                    new_a_level[action].add(pg_node_s)
#        pdb.set_trace()                    
#        for action, parents in new_a_level.items():
#            a =  PgNode_a(action)
#            a.parents.update(parents)
#            self.a_levels[level].add(a)
#            for pg_node_s in parents:
#                pg_node_s.children.add(a)
    ...
    def add_literal_level(self, level):
        ...
        for pg_node_a in self.a_levels[level - 1]:
            for s in pg_node_a.effnodes:
                if s not in new_s_level:
                    new_s_level[s] = set()
                new_s_level[s].add(pg_node_a)
        
        for s, parents in new_s_level.items():
            s.parents.update(parents)
    ...
    def update_a_mutex(self, nodeset):
        ...
        A mutex relation holds between two actions of a given level
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        #if len(set(node_a1.action.effect_add).intersection(set(node_a2.action.effect_add))) >0:
        #    return True
        #if len(set(node_a1.action.effect_rem).intersection(set(node_a2.action.effect_add))) >0:
        #    return True
        
        #for set(node_a1.action.precond_pos) 
        #if(len(list(filter(lambda item: any(x in item for x in node_a2.action.effect_rem), node_a1.action.effect_add)))!=0):
        #    return True
        #if(len(list(filter(lambda item: any(x in item for x in node_a2.action.effect_add), node_a1.action.effect_rem)))!=0):
        #    return True
        return False
        #return False
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        if len(node_a1.effnodes.intersection(node_a2.prenodes)) > 0:
            return True
        if len(node_a2.effnodes.intersection(node_a1.prenodes)) > 0:
            return True
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for pg_node_a1 in node_a1.parents:
            for pg_node_a2 in node_a2.parents:
                if(pg_node_a1.is_mutex(pg_node_a2)):
                    return True
        return False
        ...
        #print("\ntesting competing") 
        #precond = dict()
        #print("symbols of node_1", node_a1.action.name, len(node_a1.prenodes))
        #for pre in node_a1.prenodes:
        #    node_a1.action.precond_pos
        #    
        #    node_a1.action.check_precond
        #    precond[pre.symbol] = 1 if pre.is_pos else -1
        #    print(pre.symbol, pre.is_pos)
            
        #print("symbols of node_2", len(node_a2.prenodes))    
        #for pre in node_a2.prenodes:
        #    print(pre.symbol, pre.is_pos)
        #    #    print("match of symbol")
        #    if pre.symbol in precond:
        #        precond[pre.symbol] += 1 if pre.is_pos else -1

        #        if(precond[pre.symbol] == 0):
        #            return True
        #print("\nend of testing competing")
        #return False
    ...
    def negation_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s) -> bool:
        ...
        :return: bool
        """
        if (node_s1.symbol == node_s2.symbol) and (node_s1.is_pos != node_s2.is_mutex):
            return True
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        :return: bool
        """
        for pg_action_a1 in node_s1.parents:
            for pg_action_a2 in node_s2.parents:
                if pg_action_a1.is_mutex(pg_action_a2):
                    continue
                return False
    ...
    def h_levelsum(self) -> int:
        ...
        for goal in self.problem.goal:
            goals.add(PgNode_s(goal, True))

            
        for index in range(len(self.s_levels)):
            level = self.s_levels[index]
            intersect = level.intersection(goals)
            for sub_goal in intersect:
                goals.remove(sub_goal)
                level_sum += index
            if(len(goals) == 0):
                break
        return level_sum
